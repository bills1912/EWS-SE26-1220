...
import { Activity, RefreshCw, Sun, Moon, Monitor, LogOut, User, Menu, X, MapPin } from 'lucide-react';
...
export function Ticker() {
  ...
  return (
    <div style={{ overflow:'hidden', background:'var(--bg1)', borderBottom:'1px solid var(--border)', padding: isMobile ? '5px 0' : '7px 0' }}>
      <div style={{ display:'flex', whiteSpace:'nowrap', animation:'ticker 38s linear infinite' }}>
        {repeated.map((p,i) => (
          <span key={i} style={{ display:'inline-flex', alignItems:'center', gap:12, padding: isMobile ? '0 20px' : '0 36px', fontSize: isMobile ? 9 : 10, fontWeight:500, color:'var(--text2)', letterSpacing:'0.08em', textTransform:'uppercase' }}>
            {p}
...
function UserMenu({ compact = false }) {
  ...
  return (
    <div style={{ position:'relative' }}>
      <button
        onClick={() => setOpen(v => !v)}
        style={{ display:'flex', alignItems:'center', gap: compact ? 0 : 7, background:'var(--bg3)', border:'1px solid var(--border)', borderRadius:99, padding: compact ? 3 : '4px 10px 4px 6px', cursor:'pointer', transition:'all .15s' }}
      >
      ...
      {open && (
        <>
          ...
          <div style={{ position:'absolute', right:0, top:'calc(100% + 8px)', zIndex:200, background:'var(--bg2)', border:'1px solid var(--border2)', borderRadius:12, minWidth:230, maxWidth:'calc(100vw - 24px)', boxShadow:'0 8px 32px rgba(0,0,0,0.5)', animation:'fadeUp .15s ease both', overflow:'hidden' }}>
...
export function Topbar({ activeTab, setTab, allowedTabs = ALL_TABS }) {
  ...
  const visibleTabs = ALL_TABS.filter(t => allowedTabs.includes(t));

  // ── MOBILE: baris ringkas (logo + live + hamburger) + panel dropdown ──────
  if (isMobile) {
    return (
      <div style={{ background:'var(--bg1)', borderBottom:'1px solid var(--border)' }}>
        <div style={{ display:'flex', alignItems:'center', height:52, padding:'0 12px', gap:8 }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, flex:1, minWidth:0 }}>
            <div style={{ width:28, height:28, borderRadius:8, background:'linear-gradient(135deg,var(--orange) 0%,#C04010 100%)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, boxShadow:'0 0 16px rgba(232,84,28,0.35)' }}>
              <Activity size={14} color="#fff" strokeWidth={2.2}/>
            </div>
            <div style={{ minWidth:0 }}>
              <div style={{ fontSize:12, fontWeight:700, color:'var(--text1)', letterSpacing:'-0.01em', lineHeight:1.15, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>
                EWS SE2026
              </div>
              <div style={{ fontSize:8, color:'#34d399', display:'flex', alignItems:'center', gap:4 }}>
                <PulseDot color="#10b981" size={5}/>{activeTab}
              </div>
            </div>
          </div>
          ...
          <button onClick={() => setMenuOpen(v => !v)}
            style={{ display:'flex', alignItems:'center', justifyContent:'center', width:34, height:34,
                     ...
                     background: menuOpen ? 'var(--orange-dim2)' : 'var(--bg3)', cursor:'pointer', flexShrink:0 }}>
            {menuOpen ? <X size={16} color="var(--orange3)" strokeWidth={2.3}/> : <Menu size={16} color="var(--text2)" strokeWidth={2.3}/>}
          </button>
        </div>

        {menuOpen && (
          <div style={{ padding:'4px 12px 14px', borderTop:'1px solid var(--border)', animation:'fadeUp .15s ease both' }}>
            {/* Nav tabs — grid 2 kolom biar padat tapi tetap gampang dijangkau jempol */}
            <div style={{ display:'grid', gridTemplateColumns:'repeat(2, 1fr)', gap:6, margin:'10px 0' }}>
              {visibleTabs.map(t => {
                const active = activeTab === t;
                return (
                  <button key={t} onClick={() => { setTab(t); setMenuOpen(false); }}
                    style={{ padding:'10px 12px', fontSize:12.5, fontWeight:active?600:400, border:'1px solid', textAlign:'left',
                             borderColor: active ? 'rgba(232,84,28,0.35)' : 'var(--border)',
                             cursor:'pointer', borderRadius:9, transition:'all .15s',
                             background:active?'var(--orange-dim2)':'var(--bg3)', color:active?'var(--orange3)':'var(--text2)' }}>
                    {t}
                  </button>
                );
              })}
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:8, paddingTop:8, borderTop:'1px solid var(--border)' }}>
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                <MapPin size={11} color="var(--text4)" strokeWidth={2}/>
                <div style={{ flex:1 }}><KecamatanFilter/></div>
              </div>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:8 }}>
                <span style={{ fontSize:10, color:'var(--text4)' }}>Tema tampilan</span>
                <ThemeToggle/>
              </div>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:8, fontSize:10, color:'var(--text3)' }}>
                <span style={{ display:'flex', alignItems:'center', gap:5 }}><RefreshCw size={10} strokeWidth={2}/> Diperbarui</span>
                <span style={{ fontFamily:'var(--mono)' }}>{lastUpdate}</span>
              </div>
            </div>
          </div>
        )}
  ...
  // ── DESKTOP/TABLET: 1 baris penuh spt semula ───────────────────────────────
  return (
