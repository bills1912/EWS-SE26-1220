// src/components/Topbar.jsx — ticker dari MongoDB
...
const FALLBACK_TICKER = ['● EWS SE2026 · Padang Lawas Utara','Memuat data…'];
  ...
  return (
    <div style={{ display:'flex', background:'var(--bg3)', border:'1px solid var(--border)', borderRadius:8, padding:2, gap:1 }}>
      {opts.map(({ val, icon: Icon, label }) => {
        ...
        return (
          <button key={val} onClick={() => setMode(val)} title={label} style={{ display:'flex', alignItems:'center', justifyContent:'center', width:26, height:26, borderRadius:6, border:'none', cursor:'pointer', background:active?'var(--bg5)':'transparent', color:active?'var(--indigo3)':'var(--text3)', transition:'all .15s' }}>
    ...
    <div style={{ display:'flex', alignItems:'center', height:56, padding:'0 24px', gap:0, background:'var(--bg1)', borderBottom:'1px solid var(--border)' }}>
      ...
      <nav style={{ display:'flex', gap:2, flex:1, overflowX:'auto' }}>
        {TABS.map(t => {
          ...
          return <button key={t} onClick={() => setTab(t)} style={{ padding:'6px 13px', fontSize:12, fontWeight:active?600:400, border:'none', cursor:'pointer', borderRadius:8, transition:'all .15s', whiteSpace:'nowrap', background:active?'rgba(99,102,241,0.15)':'transparent', color:active?'var(--indigo3)':'var(--text3)' }}>{t}</button>;
        })}
      </nav>
