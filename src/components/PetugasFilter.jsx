/**
 * src/components/PetugasFilter.jsx
 * ──────────────────────────────
 * Custom dropdown petugas (pencacah ATAU pengawas) dengan search bar —
 * identik style/interaksi dengan DesaFilter.jsx/KecamatanFilter.jsx,
 * cuma generik: label & icon bisa disesuaikan lewat props supaya 1 komponen
 * ini bisa dipakai utk filter Pencacah maupun Pengawas.
 */
import { useState, useRef, useEffect } from 'react';
import { User, ChevronDown, X, Check, Search } from 'lucide-react';

export default function PetugasFilter({ value, onChange, list, disabled, label = 'Petugas', icon: Icon = User }) {
  ...
  const filtered = search
    ? list.filter(d => d.toLowerCase().includes(search.toLowerCase()))
    : list;
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      >
        <Icon size={11} strokeWidth={2} color="inherit"/>
        <span style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            <input
              ...
              placeholder={`Cari dari ${list.length} ${labelLower}…`}
              onKeyDown={e => {
          ...
          {/* Semua Petugas — hanya tampil kalau tidak ada search */}
          {!search && (
            <div
              onClick={() => select('')}
              style={{
              ...
              return (
                ...
                >
                  <div style={{ display:'flex', alignItems:'center', gap:7 }}>
                    <Icon size={9} strokeWidth={2} color={isActive ? 'var(--orange3)' : 'var(--text4)'}/>
                    {getLabel()}
                  </div>
          ...
          {search && (
            <div style={{
              ...
              {filtered.length} dari {list.length} {labelLower}
            </div>
          )}
        </div>
      )}
